...
//MHZ19 mhz(&mySerial);
MHZ19 mhz(SOFT_RX_PIN, SOFT_TX_PIN);
...
*/    
    ...
    String saved_mqtt_server = "10.20.200.20";
    String saved_mqtt_port = "1883";
    String saved_mqtt_user = "guest";
    String saved_mqtt_token = "guest";
    String saved_mqtt_topic = "devices/co2meter";
    ...
    memset(ppms, 0, PPMS_L);
      ...
      mhz.begin(SOFT_RX_PIN, SOFT_TX_PIN);
      mhz.setAutoCalibration(false);
      /*
        while ( mhz.isWarming() ) {
            display.drawString(0, 20, "MH-Z19 now warming up... "+mhz.getStatus());
            display.display();
...
void measureDataMHZ()
{
    measurement_t m = mhz.getMeasurement();
    ppm = m.co2_ppm;
    temp = m.temperature;
    if (ppm>100) {
     acc = 1;
